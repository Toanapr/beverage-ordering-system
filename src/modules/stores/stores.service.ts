...
import { I_STORE_REPOSITORY, type IStoreRepository } from "./repositories/store-repository.interface";
...
export class StoresService {
    ...
    async findPublicList(query: QueryStoreDto): Promise<PaginatedResponseDto<Store>> {
        const page = query.page ?? 1;
        const limit = query.limit ?? 10;
        const skip = getOffset(page, limit);
    ...
    async assertOrderable(id: string): Promise<Store> {
        ...
        if (!store.isOpen) {
            throw new ForbiddenException('Cửa hàng hiện đang đóng cửa, không thể đặt hàng');
        }
        return store;
    }
}
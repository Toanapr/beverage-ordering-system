...
import { I_STORE_REPOSITORY, IStoreRepository } from './repositories/store-repository.interface';
...
describe('StoresService', () => {
    ...
    describe('create', () => {
        ...
        it('should throw ConflictException if the store name already exists', async () => {
            ...
            await expect(
                service.create({ name: 'Trà Sữa ABC', phone: '0901234567', address: 'x' }),
    ...
    describe('findPublicList', () => {
        it('should enforce isLocked = false even if the query passes isLocked = true', async () => {
            ...
            const result = await service.findPublicList({ page: 1, limit: 10, isLocked: true } as any);
        ...
        it('should calculate the correct offset when page > 1', async () => {
            ...
            await service.findPublicList({ page: 3, limit: 10 } as any);
    ...
    describe('findPublicOneOrThrow', () => {
        it('should throw NotFoundException if the store is locked', async () => {
            ...
            await expect(service.findPublicOneOrThrow('store-1')).rejects.toThrow(NotFoundException);
        });

        it('should throw NotFoundException if the store does not exist', async () => {
            repository.findById.mockResolvedValue(null);

            await expect(service.findPublicOneOrThrow('missing')).rejects.toThrow(NotFoundException);
        });
    ...
    describe('lock / unlock', () => {
        it('lock should set isLocked = true', async () => {
            ...
            expect(repository.update).toHaveBeenCalledWith('store-1', { isLocked: true });
        ...
        it('unlock should set isLocked = false', async () => {
            ...
            expect(repository.update).toHaveBeenCalledWith('store-1', { isLocked: false });
    ...
    describe('assertOrderable', () => {
        it('should throw ForbiddenException if the store is locked', async () => {
            ...
            await expect(service.assertOrderable('store-1')).rejects.toThrow(ForbiddenException);
        });
    ...
    describe('update', () => {
        ...
        it('should throw ConflictException if the new name duplicates another store name', async () => {
            ...
            await expect(service.update('store-1', { name: 'Trùng Tên' })).rejects.toThrow(ConflictException);
        });

        it('should not check for name duplication if the name is unchanged', async () => {
            repository.findById.mockResolvedValue(mockStore);
            repository.update.mockResolvedValue({ ...mockStore, phone: '0999999999' });

            await service.update('store-1', { name: mockStore.name, phone: '0999999999' });

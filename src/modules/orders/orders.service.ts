...
import { I_ORDER_REPOSITORY, type IOrderRepository } from "./repositories/order-repository.interface";
...
export class OrdersService {
    ...
    private async buildOrderItems(
        ...
        for (const requested of dto.items) {
            ...
            if (!product) {
                throw new BadRequestException(`Product ${requested.productId} does not exist`);
            }
    ...
    private async generateUniqueOrderCode(): Promise<string> {
        ...
        throw new BadRequestException('Could not generate unique order code, please try again');
    }
